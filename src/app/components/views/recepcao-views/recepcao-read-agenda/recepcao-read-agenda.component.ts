...
@Component({
  selector: 'app-recepcao-read-agenda',
  templateUrl: './recepcao-read-agenda.component.html',
  styleUrls: ['./recepcao-read-agenda.component.css']
})
export class RecepcaoReadAgendaComponent {
   ...
   public consultas(cpf: any): void {
    this.serviceCliente.buscarPorCpf(cpf).subscribe((resposta) => {
     ...
     if(this.cliente != null){
       this.router.navigate(['recepcao/clientes/' + this.cliente.id + '/consultas']);
      } else {
